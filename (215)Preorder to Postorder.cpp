...
Node* pretopost(int pre[],int low,int high)
{
    if(low>high) return NULL;
    int mid = low+1;
    while(mid<=high&&pre[mid]<pre[low])
    {
        mid++;
    }
    mid--;
    Node* l = pretopost(pre,low+1,mid);
    Node* r = pretopost(pre,mid+1,high);
    Node* node = newNode(pre[low]);
    node->left = l;
    node->right = r;
    return node;
}
